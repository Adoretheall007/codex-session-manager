...
export function DirectoryGate(props: DirectoryGateProps) {
  if (!props.supported) {
    return (
      <div className="empty-state">
        ...
        <p>
          这个页面可以直接部署在 Netlify，但会话数据仍然读取你本机目录。当前依赖 Chromium 的
  ...
  return (
    <div className="empty-state">
      ...
      <p>
        这个站点可以由 GitHub 仓库自动部署到 Netlify，但读取的仍然是你当前电脑上的 `.codex`
        本地目录。首次使用需要手动授权目录访问，之后浏览器会尝试恢复上次目录句柄。
      </p>
      <div className="helper-list">
        <p>推荐环境：桌面版 Chrome / Edge / 其他 Chromium 浏览器。</p>
        <p>推荐选择 `C:\Users\你的用户名\.codex`，这样可以读取新版 CLI 的 `session_index.jsonl` 标题索引。</p>
